...
fh = logging.FileHandler('single_GA3.log', mode='w')
...
writter = SummaryWriter(os.path.join("Result", "afea_single_GA3"))
afea = AFEA.AFEA_dqn(afea_config.Config['AFEA'],writter)
lock = threading.Lock()
end_condition = 0
    ...
    def append(self, code, solution, fitness, idx):
        if len(self.data) >= self.size:
            ...
            if self.extend_in_ptr == self.extend_out_ptr:
                ...
                remove = random.randrange(0, len(self.data) - BATCH_SIZE)
                key, value = list(self.data.items())[remove]
...
# , "../../data/sing_GA_data.dat"
def call_EA(path, num_problem):
    p = subprocess.Popen([path, "../../data/sing_GA_data.dat", str(num_problem)], stdout=subprocess.PIPE)
    ...
    for episode in range(afea_config.N_EPISODE):
        ...
        while not end_condition:
            ...
            if len(data_batch) > 0:
                ...
                for i in range(len(request_batch)):
                    if action_batch[i] == 1:
                        ...
                        if len(res) < 1:    # lookup fail due to empty database
                            ...
                            if fitness < 0.0001:
                                dim_error += 1
                            # logger.info('\n(label_batch[i], request_batch[i][0], 1/fitness, 1 / DB.avg_fitness(label_batch[i]))
                            ...
                            if ratio < THRESHOLD:
                                ...
                                num_lookup_succ[-1] += 1
                                # data record ====================================================
                for _,data in enumerate(zip(codes.tolist(),qvalues.tolist(),rewards)):
                    logger.info('Episode {}, batch {}'.format(episode,batch_id))
                    logger.info(data)
                ...
                # data record ====================================================
                for name, layer in afea.model.QNET.model.named_parameters():
                    writter.add_histogram(name + '_weight_Q', layer.cpu().data.numpy(), batch_id)
                    #writter.add_histogram(name + '_grad', layer.grad.cpu().data.numpy(), batch_id)
                for name, layer in afea.model.autoencoder.autoencoder.named_parameters():
                    writter.add_histogram(name + '_weight_AE', layer.cpu().data.numpy(), batch_id)
                    #writter.add_histogram(name + '_grad', layer.grad.cpu().data.numpy(), batch_id)
                ...
                logger.info('************************************************************************************')
                logger.info('Episode {} Batch {}:'.format(episode,batch_id))
                logger.info('rloss = {:.6f}, closs = {:.6f}, qloss = {:.6f}'.format(reconstruction_loss,clustering_loss,q_loss))
                logger.info('Clustering Entropy: {:.5f}'.format(clustering_entropy))
                logger.info('Lookup Succ: {}, Lookup Fail: {}, EA: {}'.format(num_lookup_succ[-1], num_lookup_fail[-1], num_EA[-1]))
                ...
                logger.info('MFEA solving time: {:.3f}'.format(MFEA_end_time - MFEA_start_time))
                ...
                for i in range(afea.autoencoder.NUM_OF_CLUSTER):
                    logger.info('{:.5f} '.format(1 / DB.avg_fitness(i)))
                logger.info('\n')
                afea.autoencoder.print_dlist()
                logger.info('Waiting Requests: {}'.format(RQ.size()))
                logger.info('Database size: {}'.format(len(DB.data)))
                logger.info('************************************************************************************')
                logger.info('\n')
        ...
        for i in range(afea.autoencoder.NUM_OF_CLUSTER):
            if num_per_cluster[i] == 0:
                fitness_per_cluster[i] = 0
            else:
        ...
        logger.info('Fitness Per Type: ')
        for i in range(len(fitness_per_cluster)):
            logger.info('{:.5f} '.format(fitness_per_cluster[i]))
        logger.info('\n')
        logger.info('Look up Succ: {}, Look up Fail: {}, Direct Using EA: {}, Dim Error: {}'.format(sum(num_lookup_succ),sum(num_lookup_fail),sum(num_EA), dim_error))
        logger.info('Episode Duration: {:.5f}'.format(episode_duration))
